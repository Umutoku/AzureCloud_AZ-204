...
namespace AzQueue
{
    internal class queuestorage
    {
        ...
        {
            Response<PeekedMessage> response = await queueClient.PeekMessageAsync(); //bu fonksiyon mesajı silmez sadece bakar
        ...
        {
            Response<QueueMessage> response = await queueClient.ReceiveMessageAsync(); //bu fonksiyon mesajı siler
